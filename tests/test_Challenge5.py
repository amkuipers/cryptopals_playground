...
class TestChallenge5(unittest.TestCase):
    ...
    """
    def test_xor(self):
        # text must contain newline and no indents
        # so moved outside for now

        key = b"ICE"

        ch = Challenge3(binascii.hexlify(text))
        cr = ch.xor(binascii.hexlify(key))
        ...
        self.assertEqual(len(cr), len(expected))
